  ...
  <div class="app-container">
    <el-card class="box-card remove-padding-bottom">
      ...
      <div
        ...
        :key="item.classifyId"
      >
        <div v-if="item.process.length!==0">
          <div class="workflow-classify-title">
            {{ item.name }}
          </div>
          <div style="margin-bottom: 15px;">
            <template
              v-for="(buttonItem, buttonIndex) in item.process"
            >
              <el-tooltip
                :key="buttonItem.processId"
                effect="dark"
                placement="top"
              >
                <div slot="content">
                ...
                >
                  <el-button
                    style="width: 100%"
                    plain
                    @click="submitWorkOrder(buttonItem.processId)"
                  >
...
<script>
import { classifyProcessList } from '@/api/workflow/process'
  ...
  methods: {
    getProcessList() {
      classifyProcessList(this.listQuery).then(response => {
        this.processLists = response.data.list
      })
    },
    handleQuery() {
      this.getProcessList()
