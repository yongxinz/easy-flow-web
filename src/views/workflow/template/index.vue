  ...
  <div class="app-container">
    <el-card class="box-card">
      ...
      <el-table v-loading="loading" border :data="tplList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="ID" prop="templateId" width="120" />
        <el-table-column label="名称" prop="name" :show-overflow-tooltip="true" />
        <el-table-column label="创建者" prop="createBy" :show-overflow-tooltip="true" width="150" />
  ...
  methods: {
    ...
    getList() {
      ...
      templateList(this.listQuery).then(response => {
        ...
        this.tplList = response.data.list
    ...
    handleEdit(row) {
      ...
      templateDetails(row.templateId).then(response => {
        this.ruleForm = {
          templateId: response.data.templateId,
          name: response.data.name,
          remark: response.data.remark,
          form_structure: JSON.parse(response.data.form_structure)
        }
        this.open = true
        this.$nextTick(() => {
          this.$refs.makingform.setJSON(this.ruleForm.form_structure)
        })
      })
    },
    submitForm(formName) {
    ...
    handleSave(values) {
      if (values.list.length > 0) {
        this.ruleForm.form_structure = JSON.stringify(values)
      } else {
